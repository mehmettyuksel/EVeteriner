...
<body>
   ...
   <div class="slider">
       <center><h1>Fatura ve Ödeme Takibi</h1></center>
       <br>
       <br>
        <p>
    ...
    <center><br>
        ...
        <input type="submit" name="listele" value=" FATURA LİSTELE "  >
		<br><br>
...
{
    $toplam=0;
	$listele= $db-> query("SELECT * FROM asitest");
    $listele1= $db-> query("SELECT * FROM mamatest");
    $listele2= $db-> query("SELECT * FROM randevutest");

    $asiUcret = (int) $listele->fetch(PDO::FETCH_ASSOC)['asiucret'];
    $mamaUcret = (int) $listele1->fetch(PDO::FETCH_ASSOC)['mamaucret'];
    $randevuUcret = (int) $listele2->fetch(PDO::FETCH_ASSOC)['randevuucret'];
    $toplam = $asiUcret + $mamaUcret + $randevuUcret;

    echo "<center>
          <table border='10' bordercolor='Blue' bgcolor='#00f0f0' CELLPADDING='10' CELLSPACING='10' >
          <tr>
            <td>Aşi Ücreti: $asiUcret</td>
            <td>Mama Ücreti: $mamaUcret</td>
            <td>Randevu Ücreti: $randevuUcret</td>
          </tr>
          <tr>
            <th colspan='3'>Toplam Faturanız: $toplam</th>
          </tr>
          </table>
          </center>";
